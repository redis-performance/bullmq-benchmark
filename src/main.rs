...
mod tests {
    ...
    fn explicit_db_overrides_the_db_in_the_url() {
        // Regression: --db used to apply only when --url carried no path. The default --url always
        // has one, so --db was silently ignored -- and db 13 does not exist on Redis Cluster or
        // most managed Redis, making `--db 0` both the documented workaround and a no-op.
        let cli = Cli {
            url: "redis://127.0.0.1:6379/13".into(),
            host: None,
            port: None,
